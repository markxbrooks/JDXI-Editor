...
class PitchWheel(WheelWidget):
    ...
    def set_value(self, value):
        ...
        bend_value = int(
            (self.value + 1.0) * MidiConstant.PITCH_BEND_CENTER
        )  # Convert to 0–16383
        bend_value = max(
            0, min(MidiConstant.PITCH_BEND_RANGE, bend_value)
        )  # Clamp to [0, 16383]

        lsb = bend_value & BitMask.LOW_7_BITS
        msb = (bend_value >> 7) & BitMask.LOW_7_BITS
        for channel in [0, 1, 2]:
            status = MidiConstant.PITCH_BEND | (
                channel & MidiConstant.MIDI_CHANNEL_MASK
            )
