...
class ADSRPlot_new(QWidget):
    ...
    def set_values(self, envelope):
        """Update envelope values and trigger a redraw."""
        self.envelope = {
            ADSRParameter(k): v for k, v in envelope.items()
        }
        self.update()
    ...
    def paintEvent(self, event):
        ...
        painter.setFont(QFont("Consolas", 10))
        # set up padding
        left_padding, right_padding, top_padding, bottom_padding = 80, 50, 50, 80
        plot_w = w - left_padding - right_padding
        plot_h = h - top_padding - bottom_padding
        # Optionally draw axis lines and labels
        painter.setPen(axis_pen)
...
class ADSRPlot_test(QWidget):
    def __init__(self, width=400, height=400, parent=None):
        super().__init__(parent)
        self.envelope = self.default_envelope()
        self.setMinimumSize(width, height)
        self.setMaximumSize(width, height)
        self.setStyleSheet("background-color: #333333;")
        self.sample_rate = 256  # Sample rate for drawing precision
    ...
    def default_envelope(self):
        return {
            ...
            ADSRParameter.DECAY_TIME: 400,
            ADSRParameter.RELEASE_TIME: 100,
            ADSRParameter.INITIAL_LEVEL: 0.0,
            ADSRParameter.PEAK_LEVEL: 1.0,
            ADSRParameter.SUSTAIN_LEVEL: 0.8,
        }

    def set_values(self, envelope):
        """Update envelope values and trigger a redraw."""
        self.envelope = {ADSRParameter(k): v for k, v in envelope.items() if k in ADSRParameter.__members__}
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # Retrieve envelope parameters with default fallback
        attack_time = self.envelope.get(ADSRParameter.ATTACK_TIME, 100) / 1000.0
        decay_time = self.envelope.get(ADSRParameter.DECAY_TIME, 400) / 1000.0
        release_time = self.envelope.get(ADSRParameter.RELEASE_TIME, 100) / 1000.0
        initial_level = self.envelope.get(ADSRParameter.INITIAL_LEVEL, 0.0)
        peak_level = self.envelope.get(ADSRParameter.PEAK_LEVEL, 1.0)
        sustain_level = self.envelope.get(ADSRParameter.SUSTAIN_LEVEL, 0.8)

        # Convert times to sample counts
        attack_samples = int(attack_time * self.sample_rate)
        decay_samples = int(decay_time * self.sample_rate)
        sustain_samples = int(self.sample_rate * 2)  # Fixed sustain duration
        release_samples = int(release_time * self.sample_rate)

        # Construct ADSR envelope curve
        attack = np.linspace(initial_level, peak_level, attack_samples, endpoint=False)
        decay = np.linspace(peak_level, sustain_level, decay_samples, endpoint=False)
        sustain = np.full(sustain_samples, sustain_level)
        release = np.linspace(sustain_level, 0, release_samples)
        ...
        left, top, right, bottom = 80, 50, 50, 80
        plot_w, plot_h = w - left - right, h - top - bottom

        # Draw axes
        axis_pen = QPen(QColor("white"))
        painter.setPen(axis_pen)
        painter.drawLine(left, top, left, top + plot_h)  # Y-axis
        painter.drawLine(left, top + plot_h, left + plot_w, top + plot_h)  # X-axis
        ...
        points = []
        num_points = 500
        indices = np.linspace(0, len(envelope) - 1, num_points).astype(int)
        for i in indices:
            t = i / self.sample_rate  # Convert to seconds
            x = left + (t / 5) * plot_w
            y = top + plot_h - (envelope[i] * plot_h)
            points.append((x, y))
