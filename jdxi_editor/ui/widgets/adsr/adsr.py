...
class ADSR_new(QWidget):
    ...
    def __init__(self, attack_param: SynthParameter, decay_param: SynthParameter,
                 ...
                 part=None,
                 parent=None,
                 depth_param: Optional[SynthParameter] = None):
        super().__init__(parent)
        ...
        # Store ADSR parameters
        if not all([attack_param, decay_param, sustain_param, release_param]):
            raise ValueError("ADSR parameters are required")
        if depth_param:
            self.parameters = {
                'attack': attack_param,
                'decay': decay_param,
        ...
        # )
        if depth_param:
            self.depth_sb = self.create_double_spinbox(0, 1, 0.01, 0)
        self.peak_sb = self.create_double_spinbox(0, 1, 0.01, self.envelope["peak_level"])
        self.sustain_sb = self.create_double_spinbox(
            0, 1, 0.01, self.envelope["sustain_level"]
        )
        ...
        if depth_param:
            self.depth_slider = self._create_parameter_slider(depth_param, "Depth", value=0)
        ...
        self.layout.addWidget(self.release_slider, 0, 3)
        if depth_param:
            self.layout.addWidget(self.depth_slider, 0, 4)
        ...
        # Connect signals
        if depth_param:
            spin_boxes = [self.attack_sb, self.decay_sb, self.sustain_sb, self.release_sb, self.depth_sb]
        else:
            spin_boxes = [ self.attack_sb, self.decay_sb, self.sustain_sb, self.release_sb ]
        for sb in spin_boxes:
            sb.valueChanged.connect(self.valueChanged)
