"""
Centralized theme management for MxPanDDA.
...
from mxlib.core.exception.log import log_exception

class ThemeManager:
    """Centralized theme management for MxPanDDA"""

    @staticmethod
    def apply_theme(theme: str = "auto") -> bool:
        """
        ...
        try:
            ...
            if theme == "auto":
                try:
                    ...
                    log.warning("Theme detection failed, defaulting to light")
            ...
            # Fallback to old API (qdarktheme < 2.0) - only accepts "dark" or "light"
                ...
                log.info(f"Theme applied via stylesheet: {resolved_theme}")
                return True
            else:
            ...
            log_exception(ex, f"Error applying theme: {ex}")
            return False
    ...
    def get_custom_stylesheet() -> str:
        ...
        These styles complement qdarktheme and provide MxPanDDA-specific styling
        with rounded corners, improved spacing, and modern aesthetics.

        :return: Additional CSS stylesheet string
        """
        return """
        /* Custom MxPanDDA styles with rounded corners and improved spacing */
        QMainWindow {
    ...
    def apply_custom_stylesheet() -> bool:
        ...
        try:
            ...
            log_exception(ex, "Error applying custom stylesheet")
            return False

    @staticmethod
    def initialize(theme: str = "auto", apply_custom: bool = True) -> bool:
        """
        Initialize theme system with both qdarktheme and custom styles.

        This is the main entry point for theme initialization.

        :param theme: Theme mode - "auto", "light", or "dark"
        :param apply_custom: Whether to apply custom MxPanDDA stylesheet
        :return: True if initialization was successful, False otherwise
        """
        success = ThemeManager.apply_theme(theme)
        if success and apply_custom:
            ThemeManager.apply_custom_stylesheet()
        return success
