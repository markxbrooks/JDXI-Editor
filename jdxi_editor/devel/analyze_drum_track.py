...
def find_drum_track(midi_file_path: str) -> Optional[Tuple[int, dict]]:
    ...
    for analysis in track_analyses:
        ...
        print(
            f"  Drum notes ({DRUM_NOTE_MIN}-{DRUM_NOTE_MAX}): {analysis['drum_note_count']}"
        )
        if analysis["note_count"] > 0:
    ...
    if best_track["score"] > 0:
        track_name = (
            best_track["track_name"] or f"Track {best_track['track_index'] + 1}"
        )
        print(
            f"Most likely drum track: Track {best_track['track_index'] + 1} ({track_name})"
        )
        ...
        if len(tied_tracks) > 1:
            ...
            for tied in sorted(tied_tracks, key=lambda x: x["track_index"]):
                ...
                print(
                    f"  - Track {tied['track_index'] + 1} ({tied_name}): "
                    f"{tied['note_count']} notes, "
                    f"max {tied['max_simultaneous']} simultaneous"
                )
            print(
                "  Selected based on: note count, polyphony, note duration, no control changes"
            )
        ...
        print(
            f"  Channel: {sorted(best_track['channels'])[0] + 1 if best_track['channels'] else 'None'}"
        )
        print(f"  Total notes: {best_track['note_count']}")
        print(
            f"  Drum notes: {best_track['drum_note_count']} ({best_track['drum_note_count']/best_track['note_count']*100:.1f}%)"
            if best_track["note_count"] > 0
            else "  Drum notes: 0"
        )
