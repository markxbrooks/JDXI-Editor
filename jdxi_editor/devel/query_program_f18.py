...
class SysExCollector(QObject):
    ...
    def on_sysex_json(self, json_string: str) -> None:
        ...
        try:
            ...
            log.message(
                f"✓ Collected SysEx response #{len(self.collected_responses)} - ADDRESS: {address}, AREA: {temporary_area}, TONE: {synth_tone}"
            )
    ...
    def start_collection(self) -> None:
        ...
        log.message(
            f"Started collecting SysEx responses (timeout: {self.collection_timeout}s)"
        )
    ...
    def check_timeout(self) -> None:
        ...
        if elapsed >= self.collection_timeout:
            log.header_message(
                f"Collection timeout reached ({self.collection_timeout}s)"
            )
            log.message(
                f"Total SysEx responses collected: {len(self.collected_responses)}"
            )
            if len(self.collected_responses) == 0:
                log.warning(
                    "No SysEx responses were collected. The JD-Xi may not have responded, or the program may not exist."
                )
            self.save_responses()
    ...
    def save_responses(self) -> None:
        ...
        try:
            ...
            log.header_message(
                f"✓ Successfully saved {len(self.collected_responses)} SysEx responses"
            )
            ...
            if len(self.collected_responses) > 0:
                addresses = [
                    r.get(SysExSection.ADDRESS, "unknown")
...
def main():
    ...
    if not midi_helper.auto_connect_jdxi():
        log.error(
            "Could not find or connect to JD-Xi MIDI ports. Please ensure the device is connected."
        )
        return 1

    log.message(
        f"Connected to JD-Xi ports - Input: {midi_helper.in_port_name}, Output: {midi_helper.out_port_name}"
    )
    ...
    max_wait_timer.timeout.connect(
        lambda: (
