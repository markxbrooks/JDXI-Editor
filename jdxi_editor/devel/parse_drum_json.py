...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        log.message(
            "Example: python3 parse_drum_json.py tests/ceremony_from_software.msz jdxi_tone_data_19703e00.json"
        )
        log.message("\nOr list all drum files:")
        log.message(
            "Example: python3 parse_drum_json.py tests/ceremony_from_software.msz --list"
        )
        return 1
    ...
    if json_filename == "--list":
        ...
        try:
            with zipfile.ZipFile(msz_path, "r") as zip_ref:
                drum_files = [
                    f for f in zip_ref.namelist() if f.endswith(".json") and "1970" in f
                ]
    ...
    param_count = len(
        [
            ...
            if k not in ["JD_XI_HEADER", "ADDRESS", "TEMPORARY_AREA", "SYNTH_TONE"]
        ]
    )
