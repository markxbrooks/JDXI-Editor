...
class ControllerTestApp:
    def __init__(self):
        if MidiIOHelper is None:
            raise RuntimeError("mido is required for MIDI helper controller tests")
    ...
    def on_control_change(self, channel, control, value):
        ...
        print(
            f"🎛️  Control Change #{self.message_count}: Channel {channel}, CC{control}, Value {value}"
        )
    ...
    def on_program_change(self, channel, program):
        ...
        self.message_count += 1
        print(
            f"🎵 Program Change #{self.message_count}: Channel {channel}, Program {program}"
        )
