...
def main():
    ...
    log.header_message(
        f"Parsing {len(sysex_messages)} SysEx messages from Program F18..."
    )
    ...
    for i, hex_str in enumerate(sysex_messages, 1):
        ...
        if parsed:
            ...
            log.message(
                f"✓ Parsed message {i} - ADDRESS: {address}, AREA: {temporary_area}, TONE: {synth_tone}"
            )
        else:
    ...
    try:
        ...
        log.message(
            f"Parsed addresses ({len(unique_addresses)} unique): {', '.join(sorted(unique_addresses))}"
        )
