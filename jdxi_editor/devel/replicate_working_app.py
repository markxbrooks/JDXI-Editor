    ...
    def midi_callback(self, message, data):
        ...
        if len(message_data) >= 3:
            ...
            if status == 0xB0:  # Control Change
                ...
                if controller == 10:
                    print(
                        f"{timestamp_str:>8}\tFrom SINCO VMX8-Private\tControl\t{channel}\tPan (fine)\t{value}"
                    )
                elif controller == 41:
                    print(
                        f"{timestamp_str:>8}\tFrom SINCO VMX8-Private\tControl\t{channel}\tController 41\t{value}"
                    )
                else:
                    print(
                        f"{timestamp_str:>8}\tFrom SINCO VMX8-Private\tControl\t{channel}\tController {controller}\t{value}"
                    )
            else:
                print(
                    f"{timestamp_str:>8}\tFrom SINCO VMX8-Private\tOther\t{message_data}"
                )
        else:
