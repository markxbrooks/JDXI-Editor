...
class MidiInHandler(MidiIOController):
    ...
    def reopen_input_port_name(self, in_port: str) -> bool:
        ...
        try:
            if self.input_port_number is None:
                log.warning(
                    "No MIDI input port to reopen", scope=self.__class__.__name__
                )
                return False
            ...
            if hasattr(self, "midi_callback"):
                self.midi_in.set_callback(self.midi_callback)
                log.message(
                    f"Callback reattached to MIDI input port {in_port}",
    ...
    def _handle_sysex_message(self, message: mido.Message, preset_data: dict) -> None:
        ...
        try:
            ...
            try:
                ...
                log.error(
                    f"Failed to parse JD-Xi tone data: {parse_ex}",
                    scope=self.__class__.__name__,
                )
    ...
    def _handle_control_change(
        ...
        if value in [
            ...
            log.parameter(
                "control", control, scope=self.__class__.__name__
            )  # Bank Select LSB 00 or 01
            log.parameter(
                "value", value, scope=self.__class__.__name__
            )  # Bank Select LSB 00 or 01
    ...
    def _handle_program_change(self, message: mido.Message, preset_data: dict) -> None:
        ...
        log.message(
            f"Program Change - Channel: {channel}, Program: {program_number}",
            scope=self.__class__.__name__,
        )
    ...
    def _auto_add_current_program(self):
        ...
        if data.program_number is None:
            log.message(
                "No program number; cannot auto-add program",
                scope=self.__class__.__name__,
            )
            return
        ...
        try:
            ...
            if msb is None:
                log.message(
                    f"❌ Missing MSB (msb={msb}); cannot auto-add program",
                    scope=self.__class__.__name__,
                )
                return
            ...
            if msb == 85:
                if lsb == 0:
                    ...
                    log.message(
                        f"❌ Unsupported LSB {lsb} for user/extra banks",
                        scope=self.__class__.__name__,
                    )
                    return
            else:
                log.message(
                    f"❌ Unsupported MSB {msb} (expected 85)",
                    scope=self.__class__.__name__,
                )
                return
            ...
            log.message(
                f"Error {ex} creating JDXiProgram", scope=self.__class__.__name__
            )
            return
    ...
    def _emit_program_name_signal(self, area: str, tone_name: str) -> None:
        ...
        if area == AreaMSB.TEMPORARY_PROGRAM.name:
            log.message(
                f"Emitting program name: {tone_name} to {area}",
                scope=self.__class__.__name__,
            )
    ...
    def _emit_tone_name_signal(self, area: str, tone_name: str) -> None:
        ...
        if synth_type:
            ...
            log.warning(
                f"Unknown area: {area}. Cannot emit tone name.",
                scope=self.__class__.__name__,
            )
