...
class ProgramDatabase:
    ...
    def add_or_replace_program(self, program: JDXiProgram) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.commit()
                log.message(f"✅ Saved program to database: {program.id} - {program.name}")
                return True
        except Exception as e:
    ...
    def add_program_to_playlist(self, playlist_id: int, program_id: str, position: int = None) -> bool:
        """
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.commit()
                log.info(f"✅ Added program {program_id} to playlist {playlist_id} at position {position}")
                return True
        except sqlite3.IntegrityError:
            log.warning(f"⚠️ Program {program_id} already in playlist {playlist_id} at position {position}")
            return False
        except Exception as e:
            log.error(f"❌ Failed to add program {program_id} to playlist {playlist_id}: {e}")
            return False
    ...
    def remove_program_from_playlist(self, playlist_id: int, program_id: str) -> bool:
        ...
        try:
            ...
            log.error(f"❌ Failed to remove program {program_id} from playlist {playlist_id}: {e}")
            return False
    ...
    def update_playlist_item_midi_file(self, playlist_id: int, program_id: str, midi_file_path: str) -> bool:
        """
        ...
        try:
            with self._get_connection() as conn:
                conn.execute("""
                    ...
                    SET midi_file_path = ?
                    WHERE playlist_id = ? AND program_id = ?
                """, (midi_file_path if midi_file_path else None, playlist_id, program_id))
                conn.commit()
                log.info(f"✅ Updated MIDI file for playlist {playlist_id}, program {program_id}")
                return True
        except Exception as e:
        ...
        try:
            ...
            for program_dict in data:
                try:
                    ...
                    log.error(f"Error migrating program {program_dict.get('id', 'unknown')}: {e}")
