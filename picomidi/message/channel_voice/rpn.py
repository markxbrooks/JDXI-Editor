...
class RPN(Message):
    ...
    def __post_init__(self):
        ...
        if self.use_14bit:
            ...
            if not 0 <= self.value <= MidiConstant.CommonValues.VALUE_MAX_SEVEN_BIT:
                raise ValueError(f"RPN value must be between 0 and {MidiConstant.CommonValues.VALUE_MAX_SEVEN_BIT}, got {self.value}")
    ...
    def to_list(self) -> List[int]:
        ...
        messages = [
            # RPN MSB
            status, 101, param_msb,
            # RPN LSB
            status, 100, param_lsb,
            # Data Entry MSB
            status, 6, value_msb,
        ]
