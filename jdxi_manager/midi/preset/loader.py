...
class PresetLoader(QObject):
    ...
    def unsaved_changes(self, window, message):
        ...
        if self.midi_out_device:
            ...
            sysex = (
                bytes([0xF0, 0x41, 0x10, 0x00, 0x00, 0x00, 0x0E])
                + addr_bytes
                + data
                + bytes([chksum, 0xF7])
            )
    ...
    def sysex_pat_receive(self, preset_data):
        ...
        for n in range(len(preset_data["data"])):
            ...
            received_data = self.syx_receive(
                sysex_message, exp_len, preset_data["window"]
            )
            ...
            check = self.validate_pat_data(
                received_data, preset_data["pattern"][n], exp_len
            )
            if check != "ok":
                if self.debug:
                    print(check)
                ...
                self.error(
                    preset_data["window"],
                    f"Error while receiving data from JD-Xi\n\n{check}",
                )
                return
        ...
        for n in range(len(preset_data["data"])):
            self.read_pat_data(
                tmp_dump[n][12 : 12 + preset_data["datalen"][n]], preset_data, n
            )
    ...
    def validate_pat_data(self, sysex, pattern, exp_len):
        ...
        # Calculate checksum
        calc_sum = calculate_checksum(sysex[8 : exp_len - 2])
    ...
    def load_preset_old(
        ...
        try:
            ...
            logging.error(
                f"Error loading {preset_type} preset: {str(e)}", exc_info=True
            )

    @staticmethod
    def request_current_parameters(
        midi_helper: Optional[MIDIHelper], preset_type: str
    ) -> None:
