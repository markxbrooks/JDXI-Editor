from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QGroupBox, QLabel, QComboBox, QScrollArea, QPushButton
)
...
class VocalFXEditor(BaseEditor):
    ...
    def _create_vocal_effect_section(self):
        ...
        self.vocoder_synth_level.valueChanged.connect(self._on_vocoder_synth_level_changed)
    ...
    def _on_level_changed(self, value: int):
        """Handle level change"""
        if self.midi_helper:
            logging.debug(f"Sending vocal fx level change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.LEVEL:02x}, value={value:02x}")
    ...
    def _on_delay_send_changed(self, value: int):
        ...
        if self.midi_helper:
            logging.debug(f"Sending vocal fx delay send change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.DELAY_SEND:02x}, value={value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_reverb_send_changed(self, value: int):
        ...
        if self.midi_helper:
            logging.debug(f"Sending vocal fx reverb send change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.REVERB_SEND:02x}, value={value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_vocoder_switch_changed(self, checked: bool):
        ...
        if self.midi_helper:
            ...
            logging.debug(f"Sending vocoder switch change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.VOCODER_SWITCH:02x}, value={switch.midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_auto_note_changed(self, value: int):
        ...
        if self.midi_helper:
            ...
            logging.debug(f"Sending auto note switch change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.AUTO_NOTE_SWITCH:02x}, value={switch.midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_pitch_type_changed(self, index: int):
        ...
        if self.midi_helper:
            ...
            logging.debug(f"Sending auto pitch type change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.AUTO_PITCH_TYPE:02x}, value={pitch_type.midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_pan_changed(self, value: int):
        ...
        if self.midi_helper:
            ...
            midi_value = value + 64
            logging.debug(f"Sending vocal fx pan change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.PAN:02x}, value={midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_output_assign_changed(self, index: int):
        ...
        if self.midi_helper:
            ...
            logging.debug(f"Sending output assign change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.OUTPUT_ASSIGN:02x}, value={output.midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_pitch_key_changed(self, index: int):
        ...
        if self.midi_helper:
            ...
            logging.debug(f"Sending auto pitch key change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.KEY:02x}, value={key.midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_gender_changed(self, value: int):
        ...
        if self.midi_helper:
            ...
            midi_value = value + 10
            logging.debug(f"Sending gender change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.GENDER:02x}, value={midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_octave_changed(self, value: int):
        ...
        if self.midi_helper:
            logging.debug(f"Sending octave change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.OCTAVE:02x}, value={value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_balance_changed(self, value: int):
        ...
        if self.midi_helper:
            logging.debug(f"Sending balance change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.BALANCE:02x}, value={value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
    ...
    def _on_pitch_note_changed(self, index: int):
        ...
        if self.midi_helper:
            ...
            logging.debug(f"Sending auto pitch note change: area={VOCAL_FX_AREA:02x}, part={VOCAL_FX_PART:02x}, group={VOCAL_FX_GROUP:02x}, param={VocalFXParameters.NOTE:02x}, value={note.midi_value:02x}")
            self.midi_helper.send_parameter(
                area=VOCAL_FX_AREA,
                part=VOCAL_FX_PART,
                group=VOCAL_FX_GROUP,
