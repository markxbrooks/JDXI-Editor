...
class PresetEditor(QMainWindow):
    ...
    def __init__(
        ...
        if midi_helper:
            midi_helper.parameter_received.connect(
    ...
    def _filter_presets(self, search_text: str):
        ...
        if not search_text:
            ...
            filtered_indices = [
                i
                for i, preset in enumerate(self.full_preset_list)
    ...
    def _on_save_clicked(self):
        ...
        try:
            ...
            logging.debug(
                f"Saving {self.preset_type} preset to bank {bank} slot {slot}"
            )
            ...
            logging.error(
                f"Error saving {self.preset_type} preset: {str(e)}", exc_info=True
            )

    def request_current_parameters(self):
        """Request current parameters from the synth"""
        if self.midi_helper:
            PresetLoader.request_current_parameters(self.midi_helper, self.preset_type)
